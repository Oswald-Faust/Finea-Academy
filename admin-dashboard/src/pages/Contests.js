...
const Contests = () => {
  ...
  const fetchContests = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/contests?${new URLSearchParams(params)}`);
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/contests/stats/overview');
      const data = await response.json();
